...
public class Movement : MonoBehaviour
{
    ...
    if(horizontal < -horizontalthreshhold){horizontal = -horizontalthreshhold;}
    if(Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)){speed += 0.00001f;}
    if(speed > momentumthreshold){speed = momentumthreshold;}
    else{speed = basespeed;}
    if(Input.GetKeyDown(KeyCode.Space)){rb.AddForce(new Vector2(0, 5*speed), ForceMode2D.Impulse);Debug.Log("Jump");}
